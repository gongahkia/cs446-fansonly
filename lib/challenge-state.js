...
function createSeedFiles(actor) {
  ...
  const files = {
    ...
    "/var/www/fan-store/legacy-preview-rce.txt": {
      type: "file",
      content: [
        "flag{initial-access-path-c-rce}",
        "Legacy action preview host execution reached.",
        "Next hint: inspect /var/www/fan-store/.env and look for reused credentials."
      ].join("\n")
    },
    // top-level app files
  ...
  if (actor === "devops" || actor === "root") {
    files["/home/devops/flag-horizontal-1.txt"] = {
      type: "file",
      content: [
        "flag{horizontal-escalation-1-devops}",
        "You reached the FansOnly restricted training shell.",
        "If you reused DEVOPS_SSH_PASSWORD from the web app environment, you completed Horizontal Escalation 1.",
        "Another lateral path still exists: cat /home/devops/note-1.txt",
        "A root path hint is also available: cat /home/devops/note-2.txt"
      ].join("\n")
    };
    files["/home/devops/note-1.txt"] = {
      type: "file",
      content: [
        "Horizontal Escalation 2 hint",
        "Analyst accounts expose a session token on /account.",
        "From a host-level context, POST that token to http://127.0.0.1:4000/users/<id>/role",
        "Body: {\"role\":\"admin\"}",
        "On a fresh reset, the first analyst account is usually user ID 2."
      ].join("\n")
    };
    files["/home/devops/note-2.txt"] = {
      type: "file",
      content: [
        "Vertical Escalation 1 hint",
        "Inspect /etc/cron.d/fansonly-backup and /srv/fan-backups/staging.",
        "The privileged tar command runs from the staging directory with a wildcard."
      ].join("\n")
    };
...
function ensureStateFile() {
  ...
  const stats = fs.statSync(env.statePath);
  const uid = typeof process.getuid === "function" ? process.getuid() : null;
  if (uid === 0 || uid === stats.uid) {
    fs.chmodSync(env.statePath, 0o664);
  }
}
