...
function promoteToRoot(state, host, strategy) {
  const isTar = strategy === "tar";
  const flagFile = isTar ? "/root/flag-vertical-1.txt" : "/root/flag-vertical-2.txt";
  const flagValue = isTar
    ? "flag{vertical-escalation-1-tar}"
    : "flag{vertical-escalation-2-path}";
  const nextHint = isTar
    ? "Another root path still exists: inspect /usr/local/sbin/fansonly-maintenance.sh and the PATH it uses."
    : "Another root path still exists: inspect /etc/cron.d/fansonly-backup and /srv/fan-backups/staging.";
  ...
  state.virtualHosts.root = {
    ...
    files: {
      ...
      "/root/root.txt": {
        type: "file",
        content: [
          "flag{fansonly-root-compromise-simulated}",
          `Completion marker: ${flagValue}`,
          nextHint
        ].join("\n")
      },
      [flagFile]: {
        ...
        content: [
          flagValue,
          isTar
            ? "Wildcard injection privilege escalation complete."
            : "PATH hijacking privilege escalation complete.",
          nextHint
        ].join("\n")
      }
    }
...
function applyBackgroundJobs(host, state) {
  if (host.actor === "devops" && !host.flags.tarEscalated) {
    ...
    if (hasCheckpoint && hasAction) {
      ...
      return promoteToRoot(state, host, "tar");
    }
  }
  ...
  if (host.actor === "devops" && !host.flags.pathEscalated) {
    ...
    if (planted?.type === "file") {
      ...
      return promoteToRoot(state, host, "path");
    }
  }
...
export async function runShellCommand(sessionToken, command) {
  ...
  const backgroundMessage = applyBackgroundJobs(host, state);
